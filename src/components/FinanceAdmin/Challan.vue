<template>
 ...
 <button
        @click="saveImage"
        class="p-1 bg-gray-400 text-gray-100 rounded "
      >
        Download Challan
      </button>
    </div>
    <div id="challanDetails" class="w-full flex flex-col items-center mt-10 bg-white p-4 " v-if="routeDateBookings.length">
       <div class="w-2/3">
         <div
        class="
          font-nunito
              ...
              <tr v-for="booking in routeDateBookings" :key="booking">
                ...
                <td class="px-2 w-1/6">
                  <div class="flex gap-4 flex-wrap items-start">
                    <p v-for="passenger in booking.passengers" :key="passenger" class="border h-8 w-8 text-center p-1">
...
export default {
  ...
  methods: {
    ...
    loadChallan(){
      ...
      this.cashTotal = 0;
      this.onlineTotal = 0;
        getBookingsByRouteAndScheduleDate(this.selectedDepartureTime.id, this.dateSelected).then(
        (res) => {
    ...
    viewPassengers() {
      ...
      this.cashTotal = 0;
      this.onlineTotal = 0;
      console.log("LOAD BOOKINGS FOR", this.selectedDepartureTime, this.dateSelected)
      getBookingsByRouteAndScheduleDate(this.selectedDepartureTime.id, this.dateSelected).then(
        (res) => {
          console.log("PAID Bookings", res.data);
          this.routeDateBookings = res.data;
