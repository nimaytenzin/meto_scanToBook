<template>
  <div class="w-full flex justify-center">
    <div class="flex flex-col md:p-6 h-screen items-center w-11/12">
      <div
        id="journeyDetailsMobile"
        class="text-metoPrimary-900 my-2 bg-gray-200 p-2 rounded-lg w-full"
      >
        ...
        <div class="flex flex-col gap-4 w-full">
          <div class="w-full" v-for="route in matchedRoutes" :key="route">
            ...
            >
              ...
              <div class="w-full md:w-1/3 flex flex-col justify-between">
                <p class="text-xl font-bold text-metoPrimary-300 line-through">
                  Nu.{{ route.fare }}
                </p>
            ...
            >
              <div class="w-full md:w-1/3 flex flex-col justify-between">
                <div class="my-2">
                  <p class="text-xl md:text-3xl font-bold text-metoPrimary-900">
                    {{ route.departureTime }}
                  </p>
                  <p class="text-metoPrimary-800 text-xs md:text-sm">
              ...
              <div class="w-full md:w-1/3 flex flex-col justify-between">
                ...
                <div
                  class="text-left border-b py-2 text-xl text-metoPrimary-900"
                >
                  <p class="text-xl font-bold text-metoPrimary-900">
                    Nu.{{ route.fare }}
                  </p>
...
export default {
  ...
  created() {
    ...
    this.matchedRoutes.forEach((route) => {
      ...
      if(route.parentRouteId){
        plaintext= `${route.parentRouteId}|${this.dateSelected}`
      }else{
        plaintext =  `${route.id}|${this.dateSelected}`
      }
  ...
  methods: {
    addPassengerDetails(route) {
      ...
      if (route.parentRouteId) {
        ...
        bookingCreateData = {
          ...
          scheduleHash: route.parentRoute.scheduleHash,
      ...
      console.log(bookingCreateData);

      addNewBookingCounter(bookingCreateData).then((res) => {
        if (res.status === 201) {
    ...
    viewPassengers(route) {
      ...
      this.$router.push("/staff/viewPassengers");
    },
  },
};
</script>